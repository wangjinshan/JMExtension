...
@implementation UIView (JMAdaptScreen)
...
- (void)adaptScreenWidthWithType:(FTAdaptScreenWidthType)type
    ...
    if (![self isExceptViewClassWithClassArray:exceptViews]) {
        // 是否要对约束进行等比例
        BOOL adaptConstraint = ((type & FTAdaptScreenWidthTypeConstraint) || type == FTAdaptScreenWidthTypeAll);

        // 是否对字体大小进行等比例
        BOOL adaptFontSize = ((type & FTAdaptScreenWidthTypeFontSize) || type == FTAdaptScreenWidthTypeAll);

        // 是否对圆角大小进行等比例
        BOOL adaptCornerRadius = ((type & FTAdaptScreenWidthTypeCornerRadius) || type == FTAdaptScreenWidthTypeAll);
